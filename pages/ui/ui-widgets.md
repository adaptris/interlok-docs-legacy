...
In order to use the failed message retry action on this widget you need the correct config. You need a [failed-message-retrier](http://development.adaptris.net/javadocs/v3-snapshot/Interlok-API/com/adaptris/core/DefaultFailedMessageRetrier.html) configured, which will allow a failed message to be reprocessed.
