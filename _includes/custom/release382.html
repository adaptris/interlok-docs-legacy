...
The 3.8.2.2 release only contains bug fixes that relate only to the Interlok UI build.
This version reverts all the settings editor changes that were made in version 3.8.2.
...
This version has a Critical UI bug relating to the settings editor. This means you are unable to configure certain components correctly.
This is because in the 3.8.2 release, we made changes to the way the editor displays collections of objects, unfortunately, it has been a very problematic huge change and it means 3.8.2.1 is unusable for configuring components in the UI with metadata filters.

As a result of this bug, we feel it is better if we release a 3.8.2.2 version that reverts the settings editor changes to how they were pre-3.8.2.
This issue is a problem with the Interlok UI only, no other artefacts are affected.
